...
export class EmailService {
  ...
  public static async sendVerificationEmail(toEmail: string, token: string, firstName?: string): Promise<void> {
    ...
    try {
      ...
      console.error('[EmailService] Failed to find user for email logging:', dbErr);
    }
    ...
    try {
      ...
      try {
        ...
        console.error('[EmailService] Failed to write failed email log to database:', dbErr);
      }
      console.error(`[EmailService] Failed to send verification email to ${toEmail}:`, err);
      throw err;
    }
  }
  ...
  public static async sendPasswordResetEmail(toEmail: string, token: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[EmailService] Password reset email failure:`, err);
      throw err;
    }
  }
}
