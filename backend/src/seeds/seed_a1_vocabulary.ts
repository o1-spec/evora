...
async function main() {
  console.log("🌱 Seeding A1 Beginner Vocabulary Content...");

  // Load JSON file
  const jsonPath = path.join(__dirname, "a1_vocabulary.json");
  ...
  // 1. Delete the default empty 'Vocabulaire' module under A1 if it exists
  ...
  // 3. Create the 3 A1 Vocabulary modules
  for (let mIdx = 0; mIdx < modulesData.length; mIdx++) {
    ...
    for (let lIdx = 0; lIdx < modSeed.lessons.length; lIdx++) {
      ...
      for (let exIdx = 0; exIdx < lessonSeed.exercises.length; exIdx++) {
        ...
        // Match ExerciseType enum values
  ...
  // 4. Update the orderIndex of the remaining A1 modules (Grammaire, Dialogues, Écriture)
  // so they correctly follow the newly inserted vocabulary modules (indices 1, 2, 3)
  ...
  console.log("✅ A1 Vocabulary seeding completed successfully!");
}
