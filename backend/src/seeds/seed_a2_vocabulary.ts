...
async function main() {
  console.log("🌱 Seeding A2 Elementary Vocabulary Content...");

  // Load JSON file
  const jsonPath = path.join(__dirname, "a2_vocabulary.json");
  ...
  // 1. Delete the default empty 'Vocabulaire' module under A2 if it exists
  ...
  // 3. Create the A2 Vocabulary modules
  ...
  // 4. Update the orderIndex of the remaining A2 modules (Grammaire, Dialogues, Écriture)
  ...
  let startIdx = modulesData.length + 1; // starts at 4
  ...
  console.log("✅ A2 Vocabulary seeding completed successfully!");
}
